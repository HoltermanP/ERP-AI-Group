...
export function buildDefaultInvoiceTerms(company: CompanyProfile | null): string {
  ...
  const core = iban
    ? `Graag het totaalbedrag binnen 30 dagen na factuurdatum overmaken op bankrekening ${iban} t.n.v. ${name}, onder vermelding van het factuurnummer.`
    : `Graag het totaalbedrag binnen 30 dagen na factuurdatum overmaken, onder vermelding van het factuurnummer. (Vul uw IBAN in bij Instellingen om de bankregel automatisch te tonen.)`
